    ...
    SubShader
    {
        ...
        HLSLINCLUDE
            	...
            	)
			{
            	...
            	o.color.xyz = SampleSH(0) *  grassColor;
            	// apply SSS
            	...
            	float r01 = rand01(o.positionWS);
            	r01 *= 0.5f;
            	r01 += 0.5f;
            	
            	float3 H =  normalize(L + N) ;
            	float diffuse =  saturate(dot(L, N)) * 0.5 + 0.5;
            	float specular = saturate(dot(N,H));
            	specular = pow(specular,1);

            	o.color.xyz += Fresnel(terrainNormal,V,3) * mainLight.color * uv.y * specular;
